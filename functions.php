...
function connect_2_db(){
	...
	try{
		$conn_instance = new PDO('mysql:host=localhost;dbname=my_blog','root','');
	}
...
function fetch_user_pics($width, $height){
	...
	while ($file = readdir($dir)) {
		...
		{
			echo "<span>
					<img style = 'padding:2%;cursor:pointer;' 
					name = 'img-name'  src='$filePath$file'  width='" .$width. "px' height='" .$height."px'/>
				 </span>";
		}
	}
}
